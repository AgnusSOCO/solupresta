...
function ValueCard({ icon, title, description, index }: ValueCardProps): ReactElement {
  return (
    <div 
      className="group relative bg-white/40 backdrop-blur-md rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-2 border border-white/20 opacity-0 animate-fade-in-up"
    ...
    >
      ...
      <div className="relative p-8">
        {/* Icon Container */}
        <div className="absolute -top-10 -right-10 w-32 h-32 bg-primary-cyan/[0.03] rounded-full transition-transform duration-500 group-hover:scale-150" />
    ...
    <section className="section-transition" aria-labelledby="needs-title">
      <div className="container relative">
        ...
        <div className="mt-16 text-center opacity-0 animate-fade-in-up" style={{ animationDelay: '800ms' }}>
          <Link 
            to="/loan-application" 
            className="inline-flex items-center bg-primary-cyan hover:bg-primary-navy text-white font-bold py-4 px-8 rounded-full transition-all duration-500 hover:scale-105 focus:ring-2 focus:ring-primary-cyan focus:ring-offset-2 focus:ring-offset-white group shadow-lg hover:shadow-primary-cyan/20"
          >
            Solicitar Ahora
            <svg 
